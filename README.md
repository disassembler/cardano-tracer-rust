...
Rust implementation of the cardano-tracer trace-forward protocol. This library allows Rust applications (like alternative Cardano nodes and tooling) to forward their traces to the standard `cardano-tracer` infrastructure for monitoring and analysis.
...
- **Acceptor**: The `cardano-tracer` service that receives traces (acts as a "client" in protocol terms)
...
```rust
use cardano_tracer::forwarder::{ForwarderConfig, TraceForwarder};
use cardano_tracer::tracer::init_tracing_with_forwarder;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = ForwarderConfig {
        socket_path: PathBuf::from("/tmp/cardano-tracer.sock"),
        queue_size: 1000,
...
```rust
use cardano_tracer::forwarder::{ForwarderConfig, TraceForwarder};
use cardano_tracer::protocol::{TraceObject, Severity, DetailLevel};
use chrono::Utc;
...
## Integration with cardano-tracer

1. Start `cardano-tracer` with a Unix socket acceptor:

```yaml
# tracer-config.yaml
network:
  - socket: /tmp/cardano-tracer.sock
```

```bash
cardano-tracer --config tracer-config.yaml
```

2. Run your Rust application configured to connect to the same socket

3. Traces will appear in `cardano-tracer`'s output (logs, metrics, RTView, etc.)
...
- **Alternative Cardano implementations**: Enable Rust-based nodes to use cardano-tracer
