//! Full integration test with cardano-tracer using Pallas multiplexer
//!
//! This example connects to cardano-tracer via the Ouroboros Network mux protocol
//! and sends trace objects.

use cardano_tracer::mux::{version_table_v1, HandshakeMessage, TraceForwardClient, PROTOCOL_HANDSHAKE, PROTOCOL_TRACE_OBJECT, PROTOCOL_EKG, PROTOCOL_DATA_POINT};
use cardano_tracer::protocol::{DetailLevel, Message, MsgTraceObjectsReply, Severity, TraceObject};
use chrono::Utc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let socket_path = PathBuf::from("/tmp/cardano-tracer.sock");
    let network_magic = 764824073; // mainnet magic

    println!("Connecting to cardano-tracer at {:?}...", socket_path);
    ...
    // Subscribe to EKG and DataPoint protocols as server (cardano-tracer will initiate)
    ...
    // Wait for request from cardano-tracer
    println!("\nWaiting for trace request from cardano-tracer...");
    ...
    println!("\nCheck /tmp/cardano-tracer-test-logs/ for received traces");
